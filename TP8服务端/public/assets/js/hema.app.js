...
(function ($) {
    ...
    $.fn.extend({
						...
						$form.ajaxSubmit({
							...
							success: function (result) {
								if (options.success === $.noop) {
								    result.code == 1 ? $.show_success(result.msg, result.url) : $.show_error(result.msg,false,result.code);
								} else {
         ...
         * 确认/拒绝
         */
        confirms: function (index, url, msg) {
            $(this).click(function () {
                var param = {};
                param[index] = $(this).attr('data-id');
                layer.confirm(msg ? msg : '是否同意？',{
                    icon: 0,
                    btn:['拒绝','同意'],
                    title: '友情提示',
                    skin: 'layui-layer-hema-ok',
                    cancel : function(){
                        // 你点击右上角 X 回调
                    },
                    btn1:function(index,layero){
    ...
    $.extend({
         ...
         * reload 是否自动刷新
         */
        show_error: function (msg,reload,code) {
            ...
            layer.alert(msg, {
                ...
                , end: function () {
                    if(code == -1){
                        $("#validCode").attr('src',STORE_URL+'passport/captcha?'+Math.random());//刷新验证码
                    }else{
        ...
        showAction: function (option) {
            ...
            var $content;
            layer.open({
                type: options.type, //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
                title: options.title, //标题
                skin: 'layui-layer-hema',//皮肤
                closeBtn: options.closeBtn, //右上角关闭按钮是否显示 1显示0不显示
                ...
                btn2: function (index) {
                    if (options.btn2.call(true, $content)) {
                        layer.close(index);
                    }
                }
            });
