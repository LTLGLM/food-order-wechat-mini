...
! function(e, t) {
	...
	l.anim = ["layer-anim-00", "layer-anim-01", "layer-anim-02", "layer-anim-03", "layer-anim-04", "layer-anim-05",
		...
		var n = this,
			...
			u = r.title ? (r.uCheck ? '<form id="my-form" class="am-form tpl-form-line-form" method="post" action="" data-am-validator><fieldset>' : "") + '<div class="layui-layer-title" style="' + (f ? r.title[1] : "") + '">' + (f ? r.title[
				0] : r.title) + "</div>" : "";
		return r.zIndex = s, t([r.shade ? '<div class="' + l.SHADE + '" id="' + l.SHADE + a + '" times="' + a +
			...
			'</div><span class="layui-layer-setwin">' + function() {
				...
				for (var t = 0, i = r.btn.length; t < i; t++){
				    if(r.uCheck && t == 1){
				        e += '<button type="submit" class="j-submit am-btn am-btn-primary am-btn-sm">'+ r.btn[t] +'</button>'
				    }else{
				       e += '<a class="' + l[6] + t + '">' + r.btn[t] + "</a>"; 
				    }
				} 
				return '<div class="' + l[6] + " layui-layer-btn-" + (r.btnAlign || "") + '">' + e + "</div>" + (r.uCheck ? '</fieldset></form>' : '')
